...
class ThumbWellLayout9key(Layout):
    """The layout of a thumb well.
    """
    def __init__(self, columns: int = 3, rows: int = 3, keyswitch: Keyswitch = MX()):
        super(ThumbWellLayout9key, self).__init__(columns=columns, rows=rows, keyswitch=keyswitch)
    ...
    def web_all(self, z_offset: float = 0, thickness: Optional[float] = None, size_adjust: Optional[XYAdjustCallback] = None, position_adjust: Optional[XYAdjustCallback] = None) -> OpenSCADObject:
        ...
        return reduce(
            ...
            chain(
                ...
                [
                    hull()(
                        self.web_corner(0, 0, left=False, top=True, **web_kwargs),
                        self.web_corner(0, -1, left=False, top=False, **web_kwargs),
                        self.web_corner(1, -1, left=True, top=False, **web_kwargs),
                        self.web_corner(1, 0, left=True, top=True, **web_kwargs),
                    ),
...
class ThumbWellLayout11key(Layout):
    """The layout of a thumb well.
    """
    def __init__(self, columns: int = 3, rows: int = 4, keyswitch: Keyswitch = MX()):
        super(ThumbWellLayout11key, self).__init__(columns=columns, rows=rows, keyswitch=keyswitch)
        ...
        return reduce(
            ...
            chain(
                (
                    self.web_top_left_of(column, row, **web_kwargs)
                    for (column, row) in self.generate_positions()
                    if column > -1 and row > -1  and (column, row) not in (
                        (0, 0),
                        (0, 1),
                    )
                ),
                (
                    self.web_left_of(column, row, **web_kwargs)
                    for (column, row) in self.generate_positions()
                    if column > -1 and (column, row) not in (
                        (0, 0),
                        (0, 1),
                    )
                ),
                ...
                [
                    hull()(
                        self.web_corner(-1, 1/2, left=False, top=True, row_span=2, **web_kwargs),
                        self.web_corner(-1, -1, left=False, top=False, **web_kwargs),
                        self.web_corner(0, -1, left=True, top=False, **web_kwargs),
                        self.web_corner(0, 0, left=True, top=True, **web_kwargs),
                    ),
                    hull()(
                        self.web_corner(-1, -1, left=True, top=False, **web_kwargs),
                        self.web_corner(-1, -1, left=False, top=False, **web_kwargs),
                        self.web_corner(-1, 1/2, left=False, top=True, row_span=2, **web_kwargs),
                        self.web_corner(-1, 1/2, left=True, top=True, row_span=2, **web_kwargs),
                    ),
                    hull()(
                        self.web_corner(-1, 1/2, left=False, top=False, row_span=2, **web_kwargs),
                        self.web_corner(0, 1, left=True, top=True, **web_kwargs),
                        self.web_corner(0, 1, left=True, top=False, **web_kwargs),
                    ),
                    hull()(
                        self.web_corner(-1, 1/2, left=False, top=False, row_span=2, **web_kwargs),
                        self.web_corner(-1, 1/2, left=False, top=True, row_span=2, **web_kwargs),
                        self.web_corner(0, 0, left=True, top=False, **web_kwargs),
                        self.web_corner(0, 1, left=True, top=True, **web_kwargs),
                    ),
                    hull()(
                        self.web_corner(-1, 1/2, left=False, top=True, row_span=2, **web_kwargs),
...
class ThumbWellLayout12key(Layout):
    """The layout of a thumb well.
    """
    def __init__(self, columns: int = 3, rows: int = 4, keyswitch: Keyswitch = MX()):
        super(ThumbWellLayout12key, self).__init__(columns=columns, rows=rows, keyswitch=keyswitch)
